...
END;
/
 SELECT MADV FROM C##QLK.DONVI WHERE INP_TENDV = TENDV;
...
/
BEGIN
    C##QLK.SP_ALL_UPDATE_HOCPHAN('ATBM', 'ATBM',4, 40,30,55,'Bo mon He thong thong tin'); 
END;
/
...
CREATE OR REPLACE PROCEDURE SP_ALL_UPDATE_DONVI(
    OLD_TENDV IN VARCHAR2,
    INP_TENDV IN VARCHAR2,
    INP_TENTDV IN VARCHAR2
) AUTHID CURRENT_USER AS
    V_TRGDV VARCHAR2(25);
    V_MADV VARCHAR2(15);
BEGIN
    SELECT MANV INTO V_TRGDV FROM C##QLK.NHANSU WHERE HOTEN = INP_TENTDV;
    SELECT MADV INTO V_MADV FROM C##QLK.DONVI WHERE TENDV = OLD_TENDV;
    UPDATE C##QLK.DONVI SET
    TENDV = INP_TENDV,
    TRGDV = V_TRGDV
    WHERE MADV = V_MADV;
    COMMIT;
