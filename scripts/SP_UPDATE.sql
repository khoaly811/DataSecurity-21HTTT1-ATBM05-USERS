...
CREATE OR REPLACE PROCEDURE SP_ALL_UPDATE_HOCPHAN(
    INP_HOCPHAN IN VARCHAR2,
    INP_SOTC IN NUMBER,
    INP_SOTCLT IN NUMBER,
    INP_SOTCTH IN NUMBER,
    INP_SOSVTD IN NUMBER,
    INP_DONVI IN VARCHAR
) AUTHID CURRENT_USER AS
    V_MADV VARCHAR2(10);
    V_MASV VARCHAR2(25);
    V_ROLE VARCHAR2(30);
...
BEGIN
    SELECT * INTO V_ROLE FROM SESSION_ROLES;
    IF v_role = 'GVU' THEN
    SELECT MADV INTO V_MADV FROM C##QLK.DONVI WHERE TENDV = INP_DONVI;
    select MAHP into V_MAHP from C##QLK.HOCPHAN hp where hp.TENHP = INP_HOCPHAN;
        UPDATE C##QLK.HOCPHAN
        SET TENHP = INP_HOCPHAN,
        SOTC = INP_SOTC,
        STLT = INP_SOTCLT,
        STTH = INP_SOTCTH,
        SOSVTD = INP_SOSVTD,
