...
END;
/
SELECT SV.HOTEN AS HOTEN_SV, NS.HOTEN AS HOTEN_GV, HP.TENHP, HK, NAM, DK.MACT, DIEMTH, DIEMQT, DIEMCK, DIEMTK, DK.MASV, DK.MAGV
        FROM C##QLK.DANGKY DK JOIN C##QLK.SINHVIEN SV ON DK.MASV=SV.MASV JOIN
        C##QLK.NHANSU NS ON NS.MANV = DK.MAGV JOIN C##QLK.HOCPHAN HP ON
        HP.MAHP = DK.MAHP;
BEGIN
...
BEGIN
    ...
    FROM C##QLK.HOCPHAN HP JOIN C##QLK.KHMO KH ON HP.MAHP = KH.MAHP WHERE NGAYBD > SYSDATE - 5 AND NGAYBD < SYSDATE + 20';
    OPEN RESULT_CURSOR FOR QUERY_STRING;
END;
/
BEGIN
...
END;
/
CREATE OR REPLACE PROCEDURE SP_SV_VIEW_ADD_DANGKY(
    RESULT_CURSOR OUT SYS_REFCURSOR
) AS
    QUERY_STRING VARCHAR2(1000);
BEGIN
    QUERY_STRING := 'SELECT HP.TENHP,KH.HK, KH.NAM, KH.NGAYBD, KH.MACT, HP.MADV, HP.SOTC, HP.STLT, HP.STTH, HP.SOSVTD
    FROM C##QLK.HOCPHAN HP JOIN C##QLK.KHMO KH ON HP.MAHP = KH.MAHP WHERE KH.MAHP NOT IN (SELECT DK.MAHP FROM C##QLK.DANGKY DK) AND NGAYBD > SYSDATE - 5 AND NGAYBD < SYSDATE + 20';
    OPEN RESULT_CURSOR FOR QUERY_STRING;
END;
/

BEGIN
    SP_GRANT_EXECUTE('SP_SV_VIEW_ADD_DANGKY', 'TKHOA');
    SP_GRANT_EXECUTE('SP_SV_VIEW_ADD_DANGKY', 'TDV');
    SP_GRANT_EXECUTE('SP_SV_VIEW_ADD_DANGKY', 'GV');
    SP_GRANT_EXECUTE('SP_SV_VIEW_ADD_DANGKY', 'GVU');
    SP_GRANT_EXECUTE('SP_SV_VIEW_ADD_DANGKY', 'NVCB');
    SP_GRANT_EXECUTE('SP_SV_VIEW_ADD_DANGKY', 'SV');
END;
/
