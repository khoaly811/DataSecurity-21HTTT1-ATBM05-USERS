...
CREATE OR REPLACE PROCEDURE SP_INSERT_KHMO(
    ...
    INP_NAM INT,
    INP_MACT VARCHAR2
) AUTHID CURRENT_USER AS V_MAHP INT;
BEGIN
...
BEGIN
    ...
    SELECT MAHP INTO V_MAHP FROM C##QLK.HOCPHAN WHERE TENHP = INP_TENHP;
    SELECT MAGV INTO V_MAGV FROM C##QLK.PHANCONG WHERE MAHP=V_MAHP AND HK=INP_HK AND NAM=INP_NAM AND MACT=INP_MACT;
    INSERT INTO C##QLK.DANGKY(MASV, MAGV, MAHP,HK,NAM,MACT,DIEMTH,DIEMQT,DIEMCK,DIEMTK)
...
END;
/
CREATE OR REPLACE PROCEDURE SP_GVU_INSERT_DANGKY (
...
AS 
    ...
    V_MAHP INT;
BEGIN
    SELECT MASV INTO V_MASV FROM C##QLK.SINHVIEN WHERE HOTEN=INP_TENSV;
    SELECT MANV INTO V_MAGV FROM C##QLK.NHANSU WHERE HOTEN = INP_TENGV;
    SELECT MAHP INTO V_MAHP FROM C##QLK.HOCPHAN WHERE TENHP = INP_TENHP;
    INSERT INTO C##QLK.DANGKY(MASV, MAGV, MAHP,HK,NAM,MACT,DIEMTH,DIEMQT,DIEMCK,DIEMTK)
    VALUES (V_MASV,V_MAGV, V_MAHP, INP_HK, INP_NAM, INP_MACT, INP_TH, INP_QT, INP_CK, INP_TK);
    COMMIT;
EXCEPTION
    WHEN OTHERS THEN
        ROLLBACK;
...
/
BEGIN
    SP_GRANT_EXECUTE('SP_GVU_INSERT_DANGKY', 'TKHOA');
    SP_GRANT_EXECUTE('SP_GVU_INSERT_DANGKY', 'TDV');
    SP_GRANT_EXECUTE('SP_GVU_INSERT_DANGKY', 'GV');
    SP_GRANT_EXECUTE('SP_GVU_INSERT_DANGKY', 'GVU');
    SP_GRANT_EXECUTE('SP_GVU_INSERT_DANGKY', 'NVCB');
    SP_GRANT_EXECUTE('SP_GVU_INSERT_DANGKY', 'SV');
END;
/
...
BEGIN
    ...
    VALUES (V_MASV, INP_HOTEN, INP_PHAI, INP_NGSINH, INP_DCHI, INP_DT, INP_MACT, INP_MANGANH, INP_SOTCTL, INP_DTB);
    COMMIT;
