...
BEGIN
    ...
    SELECT PC.MAGV, PC.MAHP INTO V_MAGV, V_MAHP FROM C##QLK.PHANCONG PC JOIN C##QLK.NHANSU NS ON NS.MANV = PC.MAGV
    JOIN C##QLK.HOCPHAN HP ON HP.MAHP = PC.MAHP WHERE NS.HOTEN = INP_TENGV AND HP.TENHP = INP_TENHP
    AND INP_HK = PC.HK AND INP_NAM = PC.NAM AND INP_MACT = PC.MACT;
...
AS
    ...
    V_MAHP INT;
BEGIN
    SELECT MANV INTO V_MAGV FROM C##QLK.NHANSU WHERE VAITRO != 'Nhan vien co ban' AND INP_TENGV = HOTEN;
    SELECT KH.MAHP INTO V_MAHP FROM C##QLK.HOCPHAN HP JOIN C##QLK.KHMO KH ON HP.MAHP = KH.MAHP  WHERE INP_TENHP = TENHP;
