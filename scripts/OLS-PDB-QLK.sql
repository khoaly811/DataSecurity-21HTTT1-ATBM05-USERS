...
SELECT *FROM DBA_SA_LABELS; --tim nhung label tag nao ma co so xau (100000021 gi do)
EXECUTE SA_LABEL_ADMIN.DROP_LABEL('OLS_QLK', 1000000050);--xoa nhung label co so xau

------them may cai nay
--neu da lam cau h thi nho kiem tra xem cai so ma no co trung khong, trung thi lay so khac
EXECUTE SA_LABEL_ADMIN.CREATE_LABEL('OLS_QLK', 1100, 'TKHOA:HTTT,CNPM,KHMT,CNTT,TGMT,MMT:CS1,CS2');
EXECUTE SA_LABEL_ADMIN.CREATE_LABEL('OLS_QLK', 800, 'SV');
EXECUTE SA_LABEL_ADMIN.CREATE_LABEL('OLS_QLK', 810, 'TDV::CS1,CS2');
EXECUTE SA_LABEL_ADMIN.CREATE_LABEL('OLS_QLK', 820, 'GVU:HTTT,CNPM,KHMT,CNTT,TGMT,MMT:CS1,CS2');
EXECUTE SA_LABEL_ADMIN.CREATE_LABEL('OLS_QLK', 830, 'TDV:HTTT,CNPM,TGMT,MMT:CS1,CS2');
EXECUTE SA_LABEL_ADMIN.CREATE_LABEL('OLS_QLK', 840, 'TDV:CNPM,TGMT,MMT:CS2');
--xong roi thi chay lai SET_USER_LABELS cho nhung user tren kia lai
--nhac them: nho doi SET_USER_LABELS cho nhung USER moi

--cau h (neu thieu)
SELECT *FROM DBA_SA_LABELS;
SELECT *FROM SINHVIEN;

INSERT INTO THONGBAO(MATB,NOIDUNG) VALUES (10, 'DAY LA THONG BAO CHO GIAO VU CO SO 1');
INSERT INTO THONGBAO(MATB,NOIDUNG) VALUES (11, 'DAY LA THONG BAO CHO GIAO VU CNPM ,MMT CO SO 2');
INSERT INTO THONGBAO(MATB,NOIDUNG) VALUES (12, 'DAY LA THONG BAO CHO SINH VIEN ');


EXECUTE SA_LABEL_ADMIN.CREATE_LABEL('OLS_QLK', 890,'GVU::CS1');
EXECUTE SA_LABEL_ADMIN.CREATE_LABEL('OLS_QLK', 880, 'GVU:CNPM,MMT:CS2');
EXECUTE SA_LABEL_ADMIN.CREATE_LABEL('OLS_QLK', 870, 'SV');
EXECUTE SA_LABEL_ADMIN.CREATE_LABEL('OLS_QLK', 860,'GVU:HTTT,CNPM,MMT,KHMT,TGMT,CNTT:CS2');
EXECUTE SA_LABEL_ADMIN.CREATE_LABEL('OLS_QLK', 850, 'GV:KHMT,CNPM:CS1,CS2');
EXECUTE SA_LABEL_ADMIN.CREATE_LABEL('OLS_QLK', 790, 'SV:KHMT:CS1');


UPDATE C##QLK.THONGBAO SET OLS_THONGBAO = CHAR_TO_LABEL('OLS_QLK', 'GVU::CS1') WHERE MATB =10;
UPDATE C##QLK.THONGBAO SET OLS_THONGBAO = CHAR_TO_LABEL('OLS_QLK', 'GVU:CNPM,MMT:CS2') WHERE MATB =11;
UPDATE C##QLK.THONGBAO SET OLS_THONGBAO = CHAR_TO_LABEL('OLS_QLK', 'SV') WHERE MATB =12;




-- h1: gán nhãn cho ngu?i dùng là Giáo v? sao cho có th? d?c du?c t?t c? thông báo dành cho giáo v? o Co So 2


BEGIN
    SA_USER_ADMIN.SET_USER_LABELS(
        POLICY_NAME =>'OLS_QLK',
        USER_NAME => 'TIN93', --kt lai
        MAX_READ_LABEL => 'GVU:HTTT,CNPM,MMT,KHMT,TGMT,CNTT:CS2',
        MAX_WRITE_LABEL => 'GVU:HTTT,CNPM,MMT,KHMT,TGMT,CNTT:CS2'
);
END;
/

--h2:gán cho ngu?i dung la GV cua KHMT va CNPM doc duoc thong bao cua ca hai co so
...
BEGIN
    SA_USER_ADMIN.SET_USER_LABELS(
        POLICY_NAME =>'OLS_QLK',
        USER_NAME => 'HANH72', 
        MAX_READ_LABEL => 'GV:KHMT,CNPM:CS1,CS2',
        MAX_WRITE_LABEL => 'GV:KHMT,CNPM:CS1,CS2'
);
END;
/

--h3: Gán cho dong thong bao den toan the SV

BEGIN
    SA_USER_ADMIN.SET_USER_LABELS(
        POLICY_NAME =>'OLS_QLK',
        USER_NAME => 'Lockman46', -- thay nguoi
        MAX_READ_LABEL => 'SV:KHMT:CS1',
        MAX_WRITE_LABEL => 'SV:KHMT:CS1'
);
END;
/